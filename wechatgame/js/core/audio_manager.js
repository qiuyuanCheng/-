...
// Keep the player's low/medium/high setting relative, while raising every
// procedural sound's base output together for real-device playback.
const BASE_OUTPUT_MULTIPLIER = 2;
...
class AudioManager {
  ...
  ensureContext() {
    ...
    try {
      this.ctx = typeof wx !== "undefined" && wx.createWebAudioContext ? wx.createWebAudioContext() : null;
    } catch (err) {
      this.ctx = null;
    }
    return this.ctx;
  ...
  unlock() {
    ...
    if (this.unlocked && nowMs - this.lastUnlockAt < 1000) return;
  ...
  playUnlockTick(ctx) {
    ...
    osc.type = "sine";
    setParam(osc.frequency, 60, start);
    setParam(gain.gain, 0.0001, start);
    linearRamp(gain.gain, 0.0002, start + 0.01);
    linearRamp(gain.gain, 0.0001, start + 0.03);
    osc.connect(gain);
  ...
  play(eventName, point, meta) {
    if (!this.enabled || !eventName || this.failed) return;
    const ctx = this.ensureContext();
    if (!ctx) return;
    this.resumeIfNeeded(ctx);
    const profile = profileFor(eventName);
    const scheduleAt = this.audioNow(ctx);
    const throttleAt = Date.now() / 1000;
    ...
    if ((this.last[sourceKey] || -999) + minInterval > throttleAt) return;
    if (profile.globalInterval && (this.globalLast[eventName] || -999) + profile.globalInterval > throttleAt) return;
    this.last[sourceKey] = throttleAt;
    this.globalLast[eventName] = throttleAt;
    try {
      const panValue = Math.max(-0.35, Math.min(0.35, ((point && point.x) || 0) / 900));
      for (const layer of profile.layers || []) this.playLayer(layer, scheduleAt, panValue);
    } catch (err) {
      this.ctx = null;
    }
  }
  ...
  audioNow(ctx) {
    const current = Number(ctx && ctx.currentTime);
    if (Number.isFinite(current) && current > 0) {
      this.scheduleClock = Math.max(this.scheduleClock || 0, current);
      return current;
    }
    this.scheduleClock = Math.max((this.scheduleClock || 0) + 0.001, 0.001);
    return this.scheduleClock;
  }
  ...
  playTone(layer, now, panValue) {
    ...
    const volume = (layer.volume || 0.1) * randomRange(0.88, 1.08) * this.volume * BASE_OUTPUT_MULTIPLIER;
  ...
  playNoise(layer, now, panValue) {
    ...
    const volume = (layer.volume || 0.08) * randomRange(0.86, 1.06) * this.volume * BASE_OUTPUT_MULTIPLIER;
    this.applyEnvelope(gain.gain, start, duration, volume, layer.attack);
