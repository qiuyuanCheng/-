...
const { mapVertices, wallsFromVertices } = require("./map_geometry");
...
function buildWalls(map) {
  return wallsFromVertices(mapVertices(map && map.id, map));
}

function mapSideLength(map) {
  const verts = mapVertices(map && map.id, map);
  let total = 0;
  ...
  return total / Math.max(1, verts.length);
}
